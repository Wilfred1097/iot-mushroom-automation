...
if ($startDate && $endDate) {
    $sql = "SELECT * FROM overnight_parking WHERE date_in BETWEEN '$startDate' AND '$endDate'";
} else {
    // If no date range is provided, get all data
    $sql = "SELECT * FROM overnight_parking";
}
...
if ($exportFormat === 'csv') {
    ...
    header('Content-Disposition: attachment; filename="overnight_parking_data.csv"');

    // Output CSV headers
    $output = fopen('php://output', 'w');
    fputcsv($output, array('id', 'owner_name', 'dob', 'license', 'address', 'contact_num', 'vehicle_type', 'make_model', 'color', 'cert_reg', 'vin_num', 'date_in', 'time_in', 'date_out', 'time_out', 'payment_type', 'amount'));
